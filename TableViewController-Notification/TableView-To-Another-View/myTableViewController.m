...
- (void)viewDidLoad {
    ...
    self.title = @"Kisiler";
    // ***
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receiveNotification:) name:@"kisi_ekle" object:nil];
    
}

//bu metod addViewControllerdan gelen dicnitonary' i çeker:
-(void) receiveNotification: (NSNotification *) notification{

    //şimdi userInfo' yu gösteren Dictionary' yi oluşturalım.
    NSMutableDictionary *dic = notification.userInfo;
    
    //artık addViewController'dan gelen dic elimizde şimdi içerisinde olan Kisi objesini belirlediğimiz key' den çekelim :
    Kisi *k = [dic objectForKey:@"kisi"];
    
    //artık gelen k objesi elimizde şimdi kisiler dizisine bunu ekleyelim.
    [kisiler addObject:k];
    
    //tableview' i yeniliyoruz:
    [self.tableView reloadData];
    
    //bu metodu dinlemesi için didViewLoad'a absorved metodu ekliyoruz ***
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    //şimdi bizim uygulamamızda iki tane segue var hangi segue olduğunu kontrol ediyoruz eğer sginstance ise yai kişi seçildikten sonra myViewController' a gidiyorsa :
    if([segue.identifier isEqualToString:@"sginstance"]){
